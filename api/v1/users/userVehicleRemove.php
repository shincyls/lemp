...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "SELECT * from vehicle_info WHERE status='active' AND vehicle_plate='$plate' LIMIT 1;";
    ...
    if(empty($getcar)) array_push($error,"User Vehicle Not Found");
    
    if(empty($error)){
        $sql = "UPDATE vehicle_info SET `status`='inactive' WHERE `userid`=$getusr[0] AND `vehicle_plate`='$plate' LIMIT 1;";
        if($con->query($sql)){
            $response['statusCode'] = "200";
            $response['status'] = "OK";
            $response['statusMessage'] = "${plate} Has Successfully Removed";
        } else {
            $response['statusMessage'] = "Unable To Remove, Please Check Your Internet";
        }
    } else{
    ...
    // Logging Activity
    $ip = ((new DateTime())->format("Y-m-d H:i:s")).",'user_vehicle_remove','".json_encode($post)."','".json_encode($response)."'\r\n";
