...
function getBannerList($con){
  $data = array();
  $sql = "SELECT image,url,title,desc FROM banner_info WHERE start_date>=NOW() AND end_date<NOW() AND show?=1;";
  $ads = mysqli_fetch_all(mysqli_query($con, $sql));
  foreach($ads as $c){
    $result = [
      'img'=> $c[0],
      'url'=> $c[1],
...
function getChargesList($con,$user,$start="",$end=""){
  $where = "";
    ...
    $data = array();
    $sql = "SELECT * FROM charge_session a LEFT JOIN user_info b ON a.userid=b.id WHERE ".$where."b.username='$user' AND a.pay_status='PAID' ORDER BY a.created_on desc LIMIT 50;";
    $items = mysqli_query($con, $sql);
    foreach($items as $i){
      $result = [
        'tid'=>$i['paid_id'],
